...
This repo indexes members in three places: structured data on the members page, the RSS feed, and dedicated `/teams/:slug` profile pages. Update all by editing `data/members.ts` (the data source) and, if needed, `app/rss.xml/route.tsx` (image descriptions).
...
- `app/teams/[id]/page.tsx` renders the member profile page used by RSS URLs and the sitemap, and injects per-member `Person` JSON-LD. The slug is generated from `name` + `role`.
